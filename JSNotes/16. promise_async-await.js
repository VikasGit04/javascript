...
// A Promise in JavaScript represents a value that may be available now, or in the future, or never. It's essentially a placeholder for a value that will be resolved or rejected after an asynchronous operation completes. The Promise object provides a way to handle asynchronous operations in a more manageable and readable way compared to traditional callbacks.
// The Lifecycle of a Promise
// A promise has three states:
// Pending: The promise is in the process of being executed and hasn't been completed yet. This is the default state when the promise is created.
// Resolved (Fulfilled): The promise has been successfully completed, and a result is available. The operation succeeded.
// Rejected: The promise has failed, and an error or reason for failure is available.
// Syntax of a Promise
// A promise is created using the new Promise() constructor, which takes an executor function that receives two parameters: resolve and reject.
// javascript
// Copy code
...
// 2. Promise.all()
// Promise.all() is used when you want to wait for multiple promises to resolve before proceeding. It returns a promise that resolves when all input promises have resolved or rejects as soon as one of the promises rejects.
// javascript
// Copy code
...
// 3. Promise.race()
// Promise.race() returns a promise that resolves or rejects as soon as the first of the input promises resolves or rejects. It is useful when you want to know which promise settles first.
// javascript
// Copy code
...
// Promise.allSettled() waits for all promises to settle (either resolved or rejected) and returns an array of objects, each representing the outcome of each promise.
// javascript
// Copy code
// let promise1 = new Promise((resolve) => setTimeout(resolve, 500, "First"));
// let promise2 = new Promise((resolve, reject) => setTimeout(reject, 1000, "Second"));
...
// Promise.allSettled() returns an array where each promise has a status (either "fulfilled" or "rejected") and the value or reason.
// 5. Promise.any()
// Promise.any() returns a promise that resolves as soon as one of the input promises resolves. If all promises are rejected, it rejects with an AggregateError.
// javascript
// Copy code
...
// Promises are often paired with .catch() to handle errors. Any error thrown in the executor function (inside the promise) or in any of the .then() blocks will be caught by .catch().
// javascript
// Copy code
// let myPromise = new Promise((resolve, reject) => {
...
// Promise.all(), Promise.race(), Promise.allSettled(), and Promise.any() provide various ways to work with multiple promises concurrently.
...
// async and await are syntactic sugar for working with Promises, making asynchronous code look more like synchronous code, improving readability.
// async: This keyword is used to define a function that will always return a promise. If the function doesn't return a promise, JavaScript will wrap its return value in a promise.
// await: This keyword can only be used inside an async function. It makes JavaScript wait for the promise to resolve and then proceed with the result.
